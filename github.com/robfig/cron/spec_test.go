...
func TestDST(t *testing.T) {
	...
	// No DST for Asia/Kolkata
	loc0, _ := time.LoadLocation("Asia/Kolkata")
	...
	for _, c := range tests {
		...
		next = sched.Next(start)
		if diff := next.Sub(start); diff != c.diff {
			t.Errorf("%s, %s, %s\nobtained = %v\nexpected = %v", loc0, start.Format("Mon Jan 2 2006 15:04:05 -0700"), spec, diff, c.diff)
		}
	}
}
